...
export const UPLOAD_KIND_CONFIG = {
  ...
  rc: {
    ...
    allowedMimeTypes: ALLOWED_DOCUMENT_MIME_TYPES,
    allowedExtensions: ALLOWED_DOCUMENT_EXTENSIONS,
    // Unsigned presets are usually Image-only; use image endpoint (PDF → raw via mime later).
    resourceType: 'image' as const,
    squareCrop: false,
    compressTargetWidth: 1600,
    sources: ['camera', 'gallery', 'files'] as const,
  ...
  corporateId: {
    kind: 'corporateId' as const satisfies UploadKind,
    folder: UPLOAD_FOLDERS.corporateId,
    maxBytes: DOCUMENT_MAX_BYTES,
    allowedMimeTypes: ALLOWED_DOCUMENT_MIME_TYPES,
    allowedExtensions: ALLOWED_DOCUMENT_EXTENSIONS,
    resourceType: 'image' as const,
  ...
  issueReport: {
    kind: 'issueReport' as const satisfies UploadKind,
    folder: UPLOAD_FOLDERS.issueReport,
    maxBytes: DOCUMENT_MAX_BYTES,
    allowedMimeTypes: ALLOWED_DOCUMENT_MIME_TYPES,
    allowedExtensions: ALLOWED_DOCUMENT_EXTENSIONS,
    resourceType: 'image' as const,
